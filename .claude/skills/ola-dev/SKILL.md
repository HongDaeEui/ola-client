...
Ola 프로젝트 개발 작업을 `ola-backend`, `ola-frontend`, `ola-integrator`, `ola-qa` 4명의 전문 에이전트로 분배하고 조율한다.
...
| **백엔드 전용** | API/DB/스키마 변경만 | 서브 에이전트 (ola-backend → ola-qa) |
| **프론트 전용** | UI/컴포넌트/페이지만 | 서브 에이전트 (ola-frontend → ola-qa) |
| **버그 디버깅** | 연동 오류, 타입 불일치 등 | 서브 에이전트 (ola-integrator 또는 관련 에이전트) |
...
```
오케스트레이터
├── ola-backend: API 구현 → _workspace/backend_spec.md 저장
│   ↓ (SendMessage: API 스펙 전달)
├── ola-integrator: 타입 매핑 확인 → _workspace/integration_types.md 저장
│   ↓ (SendMessage: 확정 타입 전달)
├── ola-frontend: UI 구현 (backend_spec + integration_types 참조)
│   ↓ (SendMessage: 구현 완료 알림)
└── ola-qa: 통합 검증 → _workspace/qa_report.md 저장
```

#### 서브 에이전트 (단일 도메인)
```
오케스트레이터
├── Agent(ola-backend 또는 ola-frontend, run_in_background=false)
└── Agent(ola-qa, 검증 요청)
```
...
Phase 1 — 서브 에이전트 (병렬 조사):
  ├── Agent(ola-backend, run_in_background=true): API route 설계
  └── Agent(ola-frontend, run_in_background=true): AI SDK 컴포넌트 설계
...
- **ola-backend**: `back/src/` 하위 모든 파일. NestJS 모듈, Prisma 스키마, DTO, 서비스, 컨트롤러
- **ola-frontend**: `client_front/src/` 하위 모든 파일. 페이지, 컴포넌트, 훅, 스타일
- **ola-integrator**: 타입 정의, 환경변수 검증, API shape 매핑. 비즈니스 로직 직접 구현 금지
- **ola-qa**: 읽기 + 검증 스크립트만. 코드 수정 금지
...
- 브랜치명: `feature/ola-{기능명}` 또는 `fix/ola-{버그명}`
- 완료 후 `main`으로 merge
...
3. ola-backend: `POST /api/recommend` 엔드포인트 구현
4. ola-integrator: 요청/응답 타입 정의
5. ola-frontend: 챗봇 컴포넌트 구현 (`"use client"`, Vercel AI SDK)
6. ola-qa: API shape 매핑 검증, 다크모드 클래스 검증
7. 결과 보고

### 에러 흐름 — 백엔드 API 실패
1. ola-backend 구현 완료 → ola-qa 검증
2. qa_report: WebSocket 조건부 설정 누락 발견
3. ola-backend에 수정 요청 → 재구현
4. ola-qa 재검증 → 통과 → 보고
