...
Ola 프론트엔드(`/Users/dobedub/Documents/source/ola/client_front/`) 코드 작성 및 수정을 전담한다.
...
### 환경변수
- `NEXT_PUBLIC_API_URL` = `https://ola-backend-psi.vercel.app/api` (**반드시 `/api` suffix 포함**)
...
├── page.tsx (홈)
├── tools/, prompts/, community/, labs/, resources/, ranking/, categories/
├── meetups/, profile/, search/, submit/, about/, admin/
...
- **수신**: 오케스트레이터로부터 기능 요구사항, `ola-integrator`로부터 확정된 API 스펙
- **발신**: `ola-integrator`에게 프론트엔드에서 필요한 API shape 질문/확인 요청
- **발신**: `ola-qa`에게 구현 완료 후 검증 요청
