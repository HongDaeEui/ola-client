...
Ola 백엔드(`/Users/dobedub/Documents/source/ola/back/`) 코드 작성 및 수정을 전담한다.

## Ola 백엔드 필수 지식

### 프로젝트 구조
- **루트**: `back/src/` — 13개 NestJS 모듈 (`tools`, `meetups`, `resources`, `labs`, `prompts`, `posts`, `comments`, `likes`, `bookmarks`, `search`, `notifications`, `prisma`, app 루트)
...
### 도메인 특이사항
- **Polymorphic Like/Bookmark**: `targetType`(TOOL/PROMPT/POST/LAB) + `targetId` 문자열 기반. Prisma 관계가 아닌 애플리케이션 레벨 조인. `userId`는 **Supabase user ID** (Prisma User PK 아님).
- **Lab vs Experiment 명칭**: Prisma 모델명=`Experiment`, NestJS 모듈/라우트=`labs`, Like/Notification targetType=`"LAB"`. 이 불일치를 코드에서 반드시 일관 유지.
...
- **발신**: `ola-integrator`에게 완성된 API 스펙(엔드포인트/DTO 타입) 전달
- **발신**: `ola-qa`에게 검증 요청 시 구현 완료 + 파일 경로 전달
