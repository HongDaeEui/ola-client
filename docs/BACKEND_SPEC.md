# OlaLab Backend Specification (백엔드 명세서)

## 1. 아키텍처 개요 (Architecture Overview)
**OlaLab Backend**는 올라랩의 핵심 비즈니스 로직을 처리하고 데이터베이스를 안전하게 관리하며, 클라이언트와 관리자 앱 모두에 RESTful API를 제공하는 **중앙 통합 서버**입니다.
...
## 2. 핵심 디자인 원칙
1. **견고한 모듈형 구조:** 도구(Tools), 커뮤니티(Posts), 실험실(Experiments), 인증(Auth) 등 도메인별로 완벽히 분리된 `Module-Controller-Service` 패턴을 준수합니다.
...
│   ├── seed-100-tools.ts   # 101개 AI 도구 실데이터 자동 주입 스크립트
│   └── add-labs-may2026.ts # 오프라인 모임 데이터 주입 스크립트
├── src/
│   ├── tools/              # AI 도구 탐색 도메인 (필터링, 랭킹 로직)
│   ├── posts/              # 커뮤니티 게시글 및 댓글 로직 (조회수/좋아요 처리)
│   ├── experiments/        # 2시간 실습형 AI 모임(Labs) 콘텐츠 관리
...
- `Tool`: 이름, 설명, 평점, 가격정책, 태그, 상태(PENDING/PUBLISHED) 필드를 포함합니다.
...
`/tools/ranking` 및 `/posts/ranking` 엔드포인트는 단순 시간순 정렬이 아닌, **조회수와 좋아요 수의 가중치 합산**을 통해 인기 있는 콘텐츠를 동적으로 반환합니다. 
