...
const HeroSection = dynamic(() => import('@/components/HeroSection'), {
  ssr: false,
  loading: () => <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 pt-32 pb-20 px-4 text-center" style={{ minHeight: '500px' }} />,
});
        ...
        <div className="flex gap-4 mb-4">
          <div className="w-14 h-14 rounded-xl bg-slate-100 dark:bg-slate-700 shrink-0 overflow-hidden border border-slate-100 dark:border-slate-600 group-hover:scale-105 transition-transform duration-300">
            {tool.coverUrl || getLogoUrl(tool.iconUrl) ? (
              <Image src={(tool.coverUrl || getLogoUrl(tool.iconUrl)) as string} alt={tool.name} width={56} height={56} className="object-contain w-full h-full p-1" />
            ) : (
