...
## Analysis
6개 페이지(Community, Prompts, Meetups, Tools, Profile, Ranking)에 다크모드 클래스가 전혀 없다.
Labs 페이지(dark: 클래스 21개)와 Search 페이지가 기준 레퍼런스.
ThemeToggle 컴포넌트와 ThemeProvider가 이미 존재하므로 인프라는 완비된 상태.
Tailwind CSS v4 환경: `dark:` variant는 `.dark` 클래스 기반으로 작동.

## Approach
각 페이지의 주요 색상 토큰을 Labs 패턴에 맞춰 다크모드 variant 추가.
...
- `client_front/src/app/meetups/page.tsx`
- `client_front/src/app/tools/page.tsx`
...
- [ ] D3: meetups/page.tsx 다크모드 클래스 적용 (SuggestTopicModal은 이미 완료)
- [ ] D4: tools/page.tsx 다크모드 클래스 적용
...
|---|-------|----------|---------------|-----------|-----------|---------------------|
| 1 | Approach | Labs 패턴을 기준으로 통일 | Mechanical | DRY | 이미 검증된 패턴 재사용 | 커스텀 다크모드 컬러 신규 정의 |
