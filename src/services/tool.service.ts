...
import { Tool, GetToolsParams } from '@/models/tool';

class ToolService {
  /**
   * 1. 등록된 모든(혹은 조건별) 도구 조회
   */
  async getTools(params?: GetToolsParams): Promise<{ success: boolean; data?: { success: boolean, data: Tool[] }; error?: string }> {
   ...
   * 2. 승인 대기 중인 도구 조회
   */
  async getPendingTools(): Promise<{ success: boolean; data?: { success: boolean, data: Tool[] }; error?: string }> {
    return await apiService.callWithErrorHandling(
