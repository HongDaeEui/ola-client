...
export default async function Home() {
  ...
  return (
    <main className="pt-20 lg:pt-24 min-h-screen bg-slate-50 font-['Noto_Sans_KR']">

      {/* Hero */}
      <section className="bg-white border-b border-slate-200 pt-28 pb-20 px-4 sm:px-6 text-center">
        <div className="max-w-4xl mx-auto space-y-5">
          <p className="text-sky-600 font-semibold tracking-wider text-sm uppercase">일상을 더 창의롭게 만드는 AI 커뮤니티</p>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3 pt-2 w-full max-w-sm sm:max-w-none mx-auto">
            <Link href="/tools" className="w-full sm:w-auto bg-slate-900 text-white px-8 py-3.5 rounded-full font-bold hover:bg-slate-800 transition-all shadow-lg shadow-slate-200 text-center">
              도구 탐색
            </Link>
            <Link href="/labs" className="w-full sm:w-auto bg-white border-2 border-slate-200 text-slate-700 px-8 py-3.5 rounded-full font-bold hover:bg-slate-50 transition-all text-center">
              AI 실험실
            </Link>
            <Link href="/community/write" className="w-full sm:w-auto bg-sky-600 text-white px-8 py-3.5 rounded-full font-bold hover:bg-sky-700 transition-all shadow-lg shadow-sky-100 text-center">
              글 쓰기
            </Link>
          </div>

          {/* Search */}
          <form action="/search" method="GET" className="relative max-w-3xl mx-auto mt-8 group">
            ...
            <input
              name="q"
              type="text"
              className="w-full pl-14 pr-28 py-5 text-lg rounded-2xl border-2 border-slate-200 outline-none focus:border-sky-500 hover:border-slate-300 transition-colors shadow-sm bg-slate-50 focus:bg-white"
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-3 text-sm">
            ...
            {HINT_KEYWORDS.map(kw => (
              <Link key={kw} href={`/search?q=${encodeURIComponent(kw)}`}
                className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full hover:bg-sky-100 hover:text-sky-700 transition-colors">
                #{kw}
              </Link>
            ))}
          </div>
      ...
      <section className="max-w-[1400px] mx-auto px-6 py-10 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          {/* Tab bar */}
          <div className="flex items-center justify-between border-b border-slate-200 pb-4 mb-6 sticky top-20 bg-slate-50 z-10 pt-4">
            <div className="flex gap-1 sm:gap-4 overflow-x-auto hide-scrollbar">
              ...
              <Link href="/tools?sort=rating" className="text-slate-500 font-medium hover:text-slate-800 pb-2 whitespace-nowrap px-2 text-sm transition-colors">인기 도구</Link>
              <Link href="/tools?pricing=Free" className="text-slate-500 font-medium hover:text-slate-800 pb-2 whitespace-nowrap px-2 text-sm transition-colors">무료 도구</Link>
              <Link href="/ranking" className="text-slate-500 font-medium hover:text-slate-800 pb-2 whitespace-nowrap px-2 text-sm flex items-center gap-1 transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {gridTools.map((tool) => (
                <Link key={tool.id} href={`/tools/${tool.id}`}
                  className="bg-white border border-slate-200 rounded-2xl p-5 hover:border-sky-300 hover:shadow-lg hover:shadow-sky-100 transition-all group flex flex-col h-full">
                  <div className="flex gap-4 mb-4">
                    <div className="w-14 h-14 rounded-xl bg-slate-100 flex-shrink-0 overflow-hidden border border-slate-100 group-hover:scale-105 transition-transform duration-300">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                        <h3 className="font-bold text-slate-800 group-hover:text-sky-600 transition-colors truncate">{tool.name}</h3>
                  ...
                  <p className="text-sm font-bold text-slate-700 mb-1 line-clamp-1">{tool.shortDesc}</p>
                  <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed flex-grow">{tool.description}</p>
          ...
          <div className="mt-10 text-center">
            <Link href="/tools" className="inline-block bg-white border border-slate-300 text-slate-700 px-8 py-3 rounded-xl font-bold hover:bg-slate-50 transition-colors hover:shadow-sm">
              도구 더보기 →
            </Link>
          </div>
        </div>
        ...
        <div className="hidden lg:block w-80 flex-shrink-0 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6">
                ...
                <li key={tool.id}>
                  <Link href={`/tools/${tool.id}`} className="flex items-center gap-3 group">
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-sm text-slate-700 group-hover:text-sky-600 transition-colors truncate">{tool.name}</p>
            ...
            <Link href="/ranking" className="mt-4 flex items-center justify-center gap-1 text-xs font-bold text-slate-500 hover:text-sky-600 transition-colors pt-4 border-t border-slate-50">
              전체 랭킹 보기 <span className="material-symbols-outlined text-sm">arrow_forward</span>
            </Link>
          </div>

          {/* CTA */}
          <div className="bg-gradient-to-br from-sky-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-sky-200">
          ...
          {posts.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-2xl p-6">
                ...
                {posts.slice(0, 4).map(post => (
                  <li key={post.id}>
                    <Link href={`/community/${post.id}`} className="group block">
                      <p className="text-sm font-semibold text-slate-700 group-hover:text-sky-600 transition-colors line-clamp-2 leading-snug">{post.title}</p>
              ...
              <Link href="/community" className="mt-4 flex items-center justify-center gap-1 text-xs font-bold text-slate-500 hover:text-sky-600 transition-colors pt-4 border-t border-slate-50">
          ...
          {categories.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-2xl p-6">
              <h3 className="font-bold text-slate-800 text-base mb-4">인기 카테고리</h3>
              <div className="flex flex-wrap gap-2">
                {categories.map(cat => (
                  <Link key={cat.category} href={`/tools?category=${encodeURIComponent(cat.category)}`}
                    className="text-xs font-medium bg-slate-100 text-slate-600 px-3 py-1.5 rounded-lg hover:bg-sky-100 hover:text-sky-700 transition-colors">
                    {cat.category}
                    <span className="ml-1 text-slate-400">{cat.count}</span>
                  </Link>
                ))}
              </div>
              <Link href="/categories" className="mt-4 flex items-center justify-center gap-1 text-xs font-bold text-slate-500 hover:text-sky-600 transition-colors pt-4 border-t border-slate-50">
