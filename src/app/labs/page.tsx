            ...
            {experiments.map((exp) => {
              ...
              return (
                <Link key={exp.id} href={`/labs/${exp.id}`}
                  ...
                  {/* Banner */}
                  <div className={`h-48 relative p-6 flex flex-col justify-between overflow-hidden group-hover:shadow-[inset_0_0_80px_rgba(0,0,0,0.5)] transition-shadow duration-700 bg-slate-900`}>
                    {/* Dynamic AI Image via Pollinations */}
                    <img 
                      src={`https://image.pollinations.ai/prompt/${encodeURIComponent(`Cinematic high quality concept art background representing ${exp.category}, dark atmospheric mood, technology and AI theme. Concept: ${exp.title}`)}?width=800&height=400&nologo=true&seed=${exp.id ? exp.id.charCodeAt(0) : 1}`} 
                      alt={exp.title}
                      className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-110 group-hover:opacity-90 transition-all duration-700 ease-in-out"
                    />
