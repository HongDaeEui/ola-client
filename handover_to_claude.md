...
| 기능 | 상태 | 핵심 파일 |
|------|------|-----------|
| Labs(실험실) 목록 & 상세 | ✅ 완료 | `client_front/src/app/labs/`, `back/src/labs/` |
| Tools(도구) 목록 & 상세 | ✅ 완료 | `client_front/src/app/tools/`, `back/src/tools/` |
| 도구 → 관련 실험실 자동 연동 | ✅ 완료 | `back/src/tools/tools.service.ts` (`relatedLabs`) |
| 실험실 발표 모드 (Workshop Mode) | ✅ 완료 | `client_front/src/components/WorkshopClient.tsx` |
| 실험실 → 원클릭 모임 개설 파이프라인 | ✅ 완료 | `client_front/src/app/meetups/new/` |
...
| GET | `/api/tools/:id` | 도구 상세 (relatedLabs 포함) |
| GET | `/api/labs` | 실험실(Experiment) 목록 |
| GET | `/api/labs/:id` | 실험실 상세 |
